...
```{r}
voting <- voting |>
  mutate(red_result = if_else(state == "Wyoming" | state == "West Virginia" | state == "Utah" | state == "Tennessee" | state == "South Dakota" | state == "South Carolina" | state == "Oklahoma" | state == "North Dakota" | state == "Nebraska" | state == "Missouri" | state == "Mississippi" | state == "Louisiana" | state == "Kentucky" | state == "Kansas" | state == "Indiana" | state == "Idaho" | state == "Arkansas" | state == "Alaska" | state == "Alabama", "Red", "Blue"))
...
```
On average, according to these results, it appears that the intercept of 11.19 goes down almost a whole standard deviation when the result of Red or Blue state is made Red. This would indicate that there is a considerable positive impact on the likelihood of a state voting red if the state has a higher percentage of urban areas. It also appears that the population alone does not have much of an impact.  